...
export const useCategoryPage = () => {
  ...
  const { data, isLoading, refetch } = useGetCategories(getCategoriesReq);
  ...
  const categoriesColumns = CATEGORIES_TABLE_HEAD.map((col) => {
  ...
  const handleClick = (event: React.MouseEvent<unknown>, index: number) => {
    setSelected((prev) => {
      const selectedIndex = prev.indexOf(categories[index].id);
      let newSelected: string[] = [];

      if (selectedIndex === -1) {
        newSelected = [...prev, categories[index].id];
      } else {
        newSelected = prev.filter((id) => id !== categories[index].id);
      }
  ...
  const { mutateAsync: deleteRangeCategory } = useDeleteRangeCategories();

  const handleDeleteAll = async () => {
    await deleteRangeCategory(
      { ids: selected },
      {
        onSuccess: () => {
          refetch();
  ...
  const { mutateAsync: reActiveCategorye } = useReActiveCategory();

  const handleReActiveCategory = async (id: string) => {
    await reActiveCategorye(
